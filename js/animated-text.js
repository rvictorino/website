jQuery(document).ready(function($){
	...
	function initHeadline() {
		...
		singleLetters($('.animated-text-wrapper.letters').find('b'));
		//initialise headline animation
		animateHeadline($('.animated-text-wrapper'));
	}
	...
	function hideWord($word) {
		...
		if($word.parents('.animated-text-wrapper').hasClass('type')) {
		...
		} else if($word.parents('.animated-text-wrapper').hasClass('letters')) {
			var bool = ($word.children('i').length >= nextWord.children('i').length) ? true : false;
			hideLetter($word.find('i').eq(0), $word, bool, lettersDelay);
			showLetter(nextWord.find('i').eq(0), nextWord, bool, lettersDelay);

		}  else if($word.parents('.animated-text-wrapper').hasClass('clip')) {
			$word.parents('.animated-text').animate({ width : '2px' }, revealDuration, function(){
				switchWord($word, nextWord);
				showWord(nextWord);
			});

		} else if ($word.parents('.animated-text-wrapper').hasClass('loading-bar')){
	...
	function showLetter($letter, $word, $bool, $duration) {
		...
		if(!$letter.is(':last-child')) { 
			...
			if($word.parents('.animated-text-wrapper').hasClass('type')) { setTimeout(function(){ $word.parents('.animated-text').addClass('waiting'); }, 200);}
